...
const QrThemeCtx = createContext(darkT);
const useT = () => useContext(QrThemeCtx);

const DOT_COLORS = ["#3B82F6", "#10B981", "#F59E0B", "#8B5CF6", "#f97316", "#EF4444"];
...
const edgeColor = t =>
  t === "LEFT" ? "#10B981" : t === "RIGHT" ? "#8B5CF6" : t === "FULL" ? "#f97316" : "#3B82F6";

// ─── DB SIDEBAR GROUP ─────────────────────────────────────────────────────────
function DbGroup({ dbKey, db, nodes, onAdd, onDragStart, onExpand }) {
  const T = useT();
  const [open, setOpen] = useState(false);
  const onCanvas = name => nodes.some(n => n.name === name && n.dataSourceId === dbKey);

  const handleToggle = () => {
    const next = !open;
    setOpen(next);
    if (next && onExpand) onExpand(dbKey);
  };
    ...
    <div style={{ marginBottom: 2 }}>
      ...
      {open && (
        <div style={{ paddingLeft: 14 }}>
          {!db.tablesLoaded && (
            <div style={{ padding: "8px 10px", fontSize: 11, color: T.text3, fontStyle: "italic" }}>Loading tables…</div>
          )}
          {tableNames.map(tbl => {
            const active = onCanvas(tbl);
            const fieldCount = db.tables[tbl]?.fields?.length;
            return (
              <div key={tbl}
                draggable={!active}
              ...
              >
                ...
                {fieldCount > 0 && <span style={{ fontSize: 10, color: T.text3, fontFamily: T.mono }}>{fieldCount}c</span>}
                {active && <span style={{ fontSize: 8, color: T.accent }}>●</span>}
              </div>
            );
          })}
          {db.tablesLoaded && tableNames.length === 0 && (
            <div style={{ padding: "8px 10px", fontSize: 11, color: T.text3 }}>No tables found</div>
          )}
        </div>
      )}
    </div>
...
// ─── DROP GHOST OVERLAY ───────────────────────────────────────────────────────
function DropGhost({ x, y, tableId, dbColor }) {
  const T = useT();
  if (!tableId) return null;
  const nh = NODE_HEADER_H + 3 * FIELD_H + 8;
  return (
    <g transform={`translate(${x},${y})`} style={{ pointerEvents: "none" }}>
      <rect x={0} y={0} width={NODE_WIDTH} height={nh} rx={12}
        fill={T.accentDim} stroke={T.accent} strokeWidth={2} strokeDasharray="6 3" opacity={0.85} />
      <circle cx={16} cy={NODE_HEADER_H / 2} r={4.5} fill={dbColor || T.accent} opacity={0.7} />
...
function SqlPanel({ nodes, edges }) {
  ...
  if (!nodes.length) {
    ...
    const first = nodes[0].name || nodes[0].id;
    lines.push(<span key="s">{kw("SELECT")}{"\n  "}{tb(first)}.{cl("*")}{"\n"}</span>);
    lines.push(<span key="f">{kw("FROM")} {tb(first)}{"\n"}</span>);
    edges.forEach((e, i) => {
      lines.push(<span key={`j${i}`}>{kw(e.type + " JOIN")} {tb(nameOf(e.to))}{"\n  "}{kw("ON")} {tb(nameOf(e.from))}.{cl(e.fromField)} = {tb(nameOf(e.to))}.{cl(e.toField)}{"\n"}</span>);
    });
    lines.push(<span key="l">{kw("LIMIT")} {nm("100")}{kw(";")}</span>);
  }

  const plain = !nodes.length ? "-- Add tables to canvas" :
    `SELECT\n  ${nodes[0].name || nodes[0].id}.*\nFROM ${nodes[0].name || nodes[0].id}\n` +
    edges.map(e => `${e.type} JOIN ${nameOf(e.to)}\n  ON ${nameOf(e.from)}.${e.fromField} = ${nameOf(e.to)}.${e.toField}`).join("\n") +
    "\nLIMIT 100;";
...
// ─── JOINS PANEL ─────────────────────────────────────────────────────────────
function JoinsPanel({ edges, nodes, onChangeType, onRemove }) {
  const T = useT();
  const nameOf = id => nodes?.find(n => n.id === id)?.name || id;
      ...
      {edges.map(edge => (
        <div key={edge.id} style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: 12, marginBottom: 10, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 600, flex: 1 }}>{nameOf(edge.from)} → {nameOf(edge.to)}</span>
          ...
          <div style={{ fontFamily: T.mono, fontSize: 12, display: "flex", gap: 6, alignItems: "center", marginBottom: 10 }}>
            <span style={{ color: T.accent }}>{nameOf(edge.from)}</span><span style={{ color: T.text3 }}>.</span><span style={{ color: T.text2 }}>{edge.fromField}</span>
            <span style={{ color: T.text3 }}>──</span>
            <span style={{ color: T.accent }}>{nameOf(edge.to)}</span><span style={{ color: T.text3 }}>.</span><span style={{ color: T.text2 }}>{edge.toField}</span>
          </div>
...
function FilterEditor({ nodes }) {
  ...
  const allFields = nodes.flatMap(n => (n.cols || []).map(f => `${n.name || n.id}.${f.name}`));
  return (
    <div>
      {filters.map(f => (
...
export default function QueryBuilder() {
  ...
  /* ── Load dataset definition + columns ── */
  useEffect(() => {
    if (!api || !id) return;
    setLoading(true);
    ...
    api("/api/v1/data-sources/", {}, { limit: 100 })
      .then((data) => {
        ...
        list.forEach((ds, i) => {
          sources[ds.id] = {
            label: ds.name,
            type: ds.type_label || ds.type || "Database",
  ...
  /* ── Load tables when a data source is expanded in sidebar ── */
  const loadTablesForSource = useCallback((dsId) => {
    if (!api || dbSources[dsId]?.tablesLoaded) return;
    api(`/api/v1/data-sources/${dsId}/tables`)
      .then((tables) => {
        const tableList = Array.isArray(tables) ? tables : [];
        const tableMap = {};
        tableList.forEach((t) => {
          const name = t.name || t.table_name;
          if (name) tableMap[name] = { fields: [], schema: t.schema || "dbo" };
        });
        setDbSources((prev) => ({
          ...prev,
          [dsId]: { ...prev[dsId], tables: tableMap, tablesLoaded: true },
        }));
      })
      .catch(() => { });
  }, [api, dbSources]);
  ...
  const addTable = useCallback((name, dbKey, canvasX, canvasY) => {
    if (nodesRef.current.find(n => n.name === name && n.dataSourceId === dbKey)) return;
    let x, y;
    ...
    const newId = uid();
    const db = dbSources[dbKey];
    const schema = db?.tables?.[name]?.schema || "dbo";
    setNodes(p => [...p, { id: newId, name, schema, db: dbKey, dataSourceId: dbKey, dataSource: db?.label || "Unknown", dotColor: db?.color || T.accent, cols: [], x, y }]);
    if (api && dbKey) {
      api(`/api/v1/data-sources/${dbKey}/tables/${schema}/${name}/columns`)
        .then((cols) => {
          const normalized = (Array.isArray(cols) ? cols : []).map((c) => ({
            name: c.column_name,
            type: (c.data_type || "TEXT").toUpperCase(),
          }));
          setNodes((prev) => prev.map((n) => (n.id === newId ? { ...n, cols: normalized } : n)));
        })
        .catch(() => { });
    }
  }, [dbSources, api]);
  ...
  const handleCanvasDrop = useCallback((e) => {
    ...
    if (nodesRef.current.find(n => n.name === tableId && n.dataSourceId === dbKey)) {
      setDraggingTable(null);
      return;
    }
  ...
  if (loading) return (
    <QrThemeCtx.Provider value={T}>
      <div style={{ display: "flex", height: "100vh", background: T.bg, fontFamily: T.sans, color: T.text }}>
        <Sidebar navItems={navItems} activeNav={activeNav} onNavClick={handleNavClick} minimized={true} logo={logo} />
        <div style={{ flex: 1, display: "grid", placeItems: "center" }}>
          <div style={{ textAlign: "center", color: T.text3 }}>
            <div style={{ width: 32, height: 32, border: `3px solid ${T.border2}`, borderTopColor: T.accent, borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 16px" }} />
            <div style={{ fontSize: 14 }}>Loading definition…</div>
          </div>
        </div>
      </div>
    </QrThemeCtx.Provider>
    ...
    <QrThemeCtx.Provider value={T}>
      <div style={{ display: "flex", height: "100vh", background: T.bg, fontFamily: T.sans, color: T.text }}>
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateRows: "56px 1fr", gridTemplateColumns: "264px 1fr 310px", overflow: "hidden" }}>
          ...
          <div
            style={{
              position: "relative", overflow: "hidden", background: T.bg, cursor: conn ? "crosshair" : "default",
            ...
            >
              ...
              <g transform={`translate(${pan.x},${pan.y}) scale(${scale})`}>
                ...
                {isDragOver && dropGhost && draggingTable && !nodesRef.current.find(n => n.name === draggingTable.tableId && n.dataSourceId === draggingTable.dbKey) && (
                  <DropGhost x={dropGhost.x} y={dropGhost.y} tableId={draggingTable.tableId} dbColor={dbSources[draggingTable.dbKey]?.color} />
                )}
              </g>
            </svg>

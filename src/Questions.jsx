...
const STATUS_COLORS = {
  draft:     { bg: "rgba(250,204,21,.15)",  color: "#facc15", border: "rgba(250,204,21,.3)" },
  published: { bg: "rgba(74,222,128,.15)",  color: "#4ade80", border: "rgba(74,222,128,.3)" },
  archived:  { bg: "rgba(148,163,184,.12)", color: "#94a3b8", border: "rgba(148,163,184,.25)" },
};

const CHART_PALETTE = ["#818cf8","#34d399","#f472b6","#fb923c","#a78bfa","#38bdf8","#4ade80","#fbbf24","#f87171","#22d3ee"];
function colorForType(t) {
  if (!t) return CHART_PALETTE[0];
  let h = 0; for (const c of t) h = (h * 31 + c.charCodeAt(0)) & 0xffffff;
...
function ConfirmDialog({ title, message, confirmLabel, danger, onConfirm, onCancel, loading }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,.55)", backdropFilter: "blur(4px)" }} onClick={onCancel}>
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] shadow-2xl w-full max-w-sm mx-4" style={{ animation: "fadeUp .2s ease both" }} onClick={e => e.stopPropagation()}>
        <div className="px-6 pt-6 pb-2 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.2)" }}>
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t border-[var(--border)]">
          ...
          <button className={btnP} style={{ background: danger ? "#ef4444" : "var(--nav-active-bg)", opacity: loading ? 0.7 : 1 }} disabled={loading} onClick={onConfirm}>
            {loading && <Spin />}{confirmLabel || "Confirm"}
          </button>
        </div>
      </div>
    </div>
...
function GroupsModal({ api, onClose, push }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setGroups(list);
    } catch { push("Failed to load groups", "error"); }
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await api("/api/v1/questions-v2/categories", { method: "POST", body: JSON.stringify({ name: newName.trim() }) });
      push(`Group "${newName.trim()}" created`);
      setNewName("");
      setAdding(false);
      load();
    } catch { push("Failed to create group", "error"); }
  ...
  const handleDelete = async (g) => {
    setDeleting(g.id);
    try {
      await api(`/api/v1/questions-v2/categories/${g.id}`, { method: "DELETE" });
      push(`"${g.name}" deleted`);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,.55)", backdropFilter: "blur(4px)" }}>
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] shadow-2xl w-full max-w-md mx-4" style={{ animation: "fadeUp .2s ease both" }}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <h2 className="text-sm font-bold text-[var(--text)]">Manage Question Groups</h2>
          <button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text)] bg-[var(--bg-input)] border border-[var(--border)] cursor-pointer">
            <I d={ico.x} size={14} />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--border)]">
          <span className="text-xs text-[var(--text-muted)]">Groups ({groups.length})</span>
          <button className={btnP} style={{ background: "var(--nav-active-bg)" }} onClick={() => setAdding(true)}>
        ...
        {adding && (
          <div className="flex items-center gap-2 px-6 py-3 border-b border-[var(--border)] bg-[var(--bg-input)]">
            <input autoFocus className="flex-1 bg-[var(--bg-card)] border border-[var(--border)] rounded-lg px-3 py-1.5 text-sm text-[var(--text)] outline-none focus:border-[var(--nav-active)]"
              placeholder="Group name…" value={newName} onChange={e => setNewName(e.target.value)}
              onKeyDown={e => { if (e.key === "Enter") handleAdd(); if (e.key === "Escape") setAdding(false); }} />
            <button className={btnP} style={{ background: "var(--nav-active-bg)", opacity: saving ? 0.7 : 1 }} disabled={saving} onClick={handleAdd}>
              {saving ? <Spin /> : <I d={ico.check} size={13} color="#fff" sw={2.5} />} Save
            </button>
            <button className={btnS} onClick={() => { setAdding(false); setNewName(""); }}>Cancel</button>
          </div>
        )}
        ...
        <div className="overflow-y-auto max-h-72">
          {loading ? (
            <div className="flex items-center justify-center gap-2 py-10 text-[var(--text-muted)] text-xs"><Spin /> Loading…</div>
          ) : groups.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              <p className="text-sm font-semibold text-[var(--text-sub)] mb-1">No groups created yet</p>
              <button className="text-xs text-[var(--nav-active)] cursor-pointer hover:underline" onClick={() => setAdding(true)}>Create your first group</button>
            </div>
          ) : groups.map(g => (
            <div key={g.id} className="flex items-center justify-between px-6 py-3 border-b border-[var(--border)] hover:bg-[var(--bg-input)] transition-colors group">
              <div>
                <p className="text-sm font-medium text-[var(--text)]">{g.name}</p>
                {g.description && <p className="text-[0.6rem] text-[var(--text-muted)]">{g.description}</p>}
              </div>
              <button className="opacity-0 group-hover:opacity-100 w-7 h-7 rounded-lg flex items-center justify-center text-red-400 bg-[var(--bg-input)] border border-[var(--border)] cursor-pointer transition-opacity"
                disabled={deleting === g.id} onClick={() => handleDelete(g)}
                title="Delete group">
                {deleting === g.id ? <Spin /> : <I d={ico.trash} size={13} color="#f87171" />}
              </button>
            </div>
          ))}
        </div>
...
export default function Questions() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (qRes.status === "fulfilled") {
        const list = Array.isArray(qRes.value?.data) ? qRes.value.data
          : Array.isArray(qRes.value) ? qRes.value : [];
        setQuestions(list);
      } else {
        setQuestions([]); push("Failed to load questions", "error");
      }
      if (catRes.status === "fulfilled") {
        const list = Array.isArray(catRes.value?.data) ? catRes.value.data
  ...
  useEffect(() => {
    const h = e => { if (groupDropRef.current && !groupDropRef.current.contains(e.target)) setGroupDropOpen(false); };
  ...
  const filtered = questions.filter(q => {
    ...
    const matchSearch = !term || q.name?.toLowerCase().includes(term) || q.description?.toLowerCase().includes(term);
  ...
  useEffect(() => { setCurrentPage(1); }, [search, statusTab, groupFilter]);
  ...
  const counts = {
    ...
    draft:     questions.filter(q => (q.status || "draft").toLowerCase() === "draft").length,
    published: questions.filter(q => (q.status || "").toLowerCase() === "published").length,
    archived:  questions.filter(q => (q.status || "").toLowerCase() === "archived").length,
  };

  const selectedGroup = categories.find(c => String(c.id) === String(groupFilter));
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--bg)] text-[var(--text)] transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-2.5 border-b border-[var(--border)] bg-[var(--bg-card)]">
          ...
          <div className="relative w-52">
            <I d={ico.search} size={13} color="var(--text-muted)" className="absolute left-2.5 top-1/2 -translate-y-1/2" />
          ...
          <div className="flex items-center gap-0.5 ml-1">
            {STATUS_TABS.map(tab => (
              <button key={tab} onClick={() => setStatusTab(tab)}
                ...
                style={{
                  background: statusTab === tab ? "var(--nav-active-bg)" : "transparent",
          ...
          <div ref={groupDropRef} className="relative">
            <button onClick={() => setGroupDropOpen(p => !p)}
              className={`${btnS} text-xs`}
              style={{ borderColor: groupFilter !== "all" ? "var(--nav-active)" : undefined, color: groupFilter !== "all" ? "var(--nav-active)" : undefined }}>
              <I d={ico.filter} size={13} />
              {selectedGroup ? `Group: ${selectedGroup.name}` : "Filter by groups…"}
              <I d={ico.chevDown} size={11} className={`transition-transform ${groupDropOpen ? "rotate-180" : ""}`} />
            </button>
            {groupDropOpen && (
              <div className="absolute right-0 top-full mt-1 w-52 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] shadow-xl z-50 overflow-hidden" style={{ animation: "fadeUp .15s ease both" }}>
                <button className="w-full text-left px-3 py-2 text-xs hover:bg-[var(--bg-input)] transition-colors border-none cursor-pointer"
                  style={{ color: groupFilter === "all" ? "var(--nav-active)" : "var(--text)" }}
                  onClick={() => { setGroupFilter("all"); setGroupDropOpen(false); }}>
                  All Groups
                </button>
                {categories.map(c => (
                  <button key={c.id} className="w-full text-left px-3 py-2 text-xs hover:bg-[var(--bg-input)] transition-colors border-none cursor-pointer"
                    style={{ color: String(groupFilter) === String(c.id) ? "var(--nav-active)" : "var(--text)" }}
                    onClick={() => { setGroupFilter(String(c.id)); setGroupDropOpen(false); }}>
                    {c.name}
                  </button>
                ))}
                {categories.length === 0 && (
                  <p className="px-3 py-4 text-xs text-[var(--text-muted)] text-center">No groups yet</p>
                )}
              </div>
            )}
          </div>
          ...
          <button className={btnS} title="Manage Groups" onClick={() => setShowGroups(true)}>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-sm font-semibold text-[var(--text-sub)] mb-1">
                {search || statusTab !== "All" || groupFilter !== "all" ? "No matching questions" : "No questions yet"}
              </p>
              <p className="text-xs text-[var(--text-muted)] mb-4">
                {search || statusTab !== "All" || groupFilter !== "all" ? "Try adjusting your filters or search term" : "Create your first question to visualize data"}
              </p>
              {!search && statusTab === "All" && groupFilter === "all" && (
                <button className={btnP} style={{ background: "var(--nav-active-bg)" }} onClick={() => navigate("/questions/new")}>
                  <I d={ico.plus} size={14} color="#fff" sw={2.5} /> Create Question
                </button>
              )}
            </div>
          ) : (
            <div className="px-5 py-0">
                ...
                <tbody>
                  {paginated.map(q => (
                    <tr key={q.id}
                      ...
                      <td className="px-3 py-2.5">
                        <span className="text-xs text-[var(--text-muted)]">
                          {q.updated_at ? new Date(q.updated_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "Never"}
                        </span>
                      </td>
                      <td className="px-3 py-2.5 relative" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-center">
                          <button id={`q-menu-${q.id}`}
                        ...
                        {openMenu === q.id && (
                          <div className="absolute right-6 mt-1 w-44 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] shadow-lg z-50 overflow-hidden" style={{ animation: "fadeUp .15s ease both" }}>
                            <button className="flex items-center gap-2 w-full px-3 py-2 text-xs hover:bg-[var(--bg-input)] text-[var(--text)] bg-transparent border-none cursor-pointer"
                              onClick={() => navigate(`/questions/${q.id}`)}>
                            ...
                            <button className="flex items-center gap-2 w-full px-3 py-2 text-xs hover:bg-[var(--bg-input)] text-[var(--text)] bg-transparent border-none cursor-pointer"
                              ...
                              <svg className="shrink-0" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12" /></svg>
                              Materialize
                            </button>
                            <button className="flex items-center gap-2 w-full px-3 py-2 text-xs hover:bg-[var(--bg-input)] text-[var(--text)] bg-transparent border-none cursor-pointer"
                              onClick={() => push("Assign Groups — coming soon")}>
                              <I d={ico.users} size={13} /> Assign Groups
                            </button>
                            <div className="border-t border-[var(--border)] mx-2 my-1" />
                            <button className="flex items-center gap-2 w-full px-3 py-2 text-xs text-red-400 hover:bg-[var(--bg-input)] bg-transparent border-none cursor-pointer"
              ...
              <div className="mt-3 px-1 flex items-center justify-between gap-3 pb-4">
                <span className="text-[0.65rem] text-[var(--text-muted)]">
                  Showing {filtered.length === 0 ? 0 : pageStart + 1}–{Math.min(pageStart + PAGE_SIZE, filtered.length)} of {filtered.length} question{filtered.length !== 1 ? "s" : ""}
                </span>
      ...
      {/* ── Modals ── */}
      {showGroups && <GroupsModal api={api} push={push} onClose={() => { setShowGroups(false); fetchAll(); }} />}
      {deleting && (
        <ConfirmDialog title="Delete Question"
